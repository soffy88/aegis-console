...
export default function AlertIngestPage() {
  ...
  const mutation = useMutation({
    mutationFn: (body: Record<string, unknown>) =>
      aegisFetch<unknown>("/api/v1/alerts/ingest", {
        method: "POST",
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    try {
      extraPayload = JSON.parse(form.payload);
    } catch {
      extraPayload = {};
    }
    const body = {
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm opacity-70">
        Fire a test alert directly to the ingest endpoint. Useful for verifying
        Brain pipeline wiring.
      </p>
      ...
      <form onSubmit={handleSubmit} className="max-w-lg space-y-4">
        ...
        <button
          type="submit"
          disabled={mutation.isPending || !form.alert_name}
