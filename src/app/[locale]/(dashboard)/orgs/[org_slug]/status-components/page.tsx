...
export default function StatusComponentsPage() {
  ...
  const delM = useMutation({ mutationFn: (id: string) => aegisFetch(paths.statusComponent(orgId!, id), { method: "DELETE" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["statusComponents", orgId] }) });
    ...
    <div className="space-y-4">
      ...
      <table className="w-full text-sm"><tbody>{(q.data?.components ?? []).map((c) => (
        <tr key={c.id} className="border-b border-[var(--border)]/40">
          ...
          <td className="p-2 text-right"><button onClick={() => delM.mutate(c.id)} className="text-xs text-red-400">✕</button></td>
        </tr>))}</tbody></table>
    </div>
  );
}
